...
%       aleph = 1 + nu*M(3,i)/(M(1,i)+ M(3,i));
    ...
    % Susceptible
      M(1, i + 1) = (1 - lambda)*beta*Probability*M(1, i) + alpha*M(2, i) + mu*(M(6, i) + M(7, i));
      M(2, i + 1) = lambda*beta*Probability*M(1,i) + (1-alpha)*M(2, i);
      
    % Infected
      M(3, i + 1) = (1 - beta*Probability)*M(1,i) + (1-lambda)*(1-theta)*(1-gamma)*M(3,i) +...
